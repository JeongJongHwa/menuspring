...
//		logger.info( menuVO.toString() );
	...
	public String read(MenuVO menuVO, @ModelAttribute("cri") Criteria cri, Model model, HttpServletRequest request)
			throws Exception {
		logger.info("read");
		...
		if (user != null) {
			...
			map.put("menuNumber", menuVO.getMenuNumber());
			map.put("id", user.getId());
			model.addAttribute("appraisal", service.getAppraisal(map));
		}
	...
	@RequestMapping(value = "/menu/appraisal", method = RequestMethod.POST)
	public @ResponseBody int appraisal(@RequestBody Map<String, Object> map) throws Exception {

		logger.info((String) map.get("menuNumber"));
		logger.info((String) map.get("id"));

		return service.Appraisal(map);
	}
